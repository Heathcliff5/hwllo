...
function startMusic({ restart = false } = {}){
  ...
  if (p && typeof p.then === 'function'){
    p.then(() => {
      gsap.to(bgMusic, { volume: MUSIC_VOLUME, duration: 1.8, ease: 'power2.out' });
    }).catch(() => {
      // Browsers may still block autoplay; the arrow release is retried by the next user gesture.
    });
  }
}
...
const galleryToFilm = $('galleryToFilm');

const reduceMotion = false; // The cinematic bow intro always plays for this gift.
...
function fire(){
  ...
  drawing = false;
  stopBeat();
  // The bow release is the first real user interaction, so this is a reliable
  // autoplay point and also makes the music enter with the cinematic transition.
  startMusic();
...
function resetAll(){
  ...
  continueBtn.classList.remove('is-shown'); continueBtn.hidden = true;
  closeGallery();
...
/* ============================================================
   ACT 5 — THE GALLERY
   A quiet, autoplaying room of photographs. Opens from the bridge
   button once the wish has landed; closes back to the tree.
   ============================================================ */
const PHOTOS = [
  { src: '/photos/memory-01.jpg', caption: 'the one that made me smile, oooo i just wanna be yoursss🥰🥰' },
  { src: '/photos/memory-02.jpg', caption: 'Damnnnn girllll😚, you slayyy!' },
  { src: '/photos/memory-03.jpg', caption: 'mine all mine 😁' },
  { src: '/photos/memory-04.jpg', caption: 'My bhondu baby🥰🥰' },
  { src: '/photos/memory-05.jpg', caption: 'mashallah habibi ye ladki humko bahut psnd hoti hum isse nikah krna chahte😚😚' },
  { src: '/photos/memory-06.jpg', caption: 'that laugh right there that just steals my heart☺️☺️' },
  { src: '/photos/memory-07.jpg', caption: 'Ohh myy GAwddd ye apne kya kiya flat leke chalo ab😁🌚' },
  { src: '/photos/memory-08.jpg', caption: 'and this one 😂😂😂' },
];

let gIndex = 0, gTimer = 0, gBuilt = false, gDots = [];
...
function buildGallery(){
  ...
  PHOTOS.forEach((p, i) => {
    ...
    fig.className = 'slide';
    // a different, gentle drift per photo so the room never feels mechanical
    const ang = (i * 47) % 360, r = 2.4;
    fig.style.setProperty('--kbx0', `${(Math.cos(ang * Math.PI / 180) * r).toFixed(2)}%`);
    fig.style.setProperty('--kby0', `${(Math.sin(ang * Math.PI / 180) * r).toFixed(2)}%`);
    fig.style.setProperty('--kbx1', '0%');
    fig.style.setProperty('--kby1', '0%');
    const img = document.createElement('img');
    img.src = p.src; img.alt = ''; img.loading = i === 0 ? 'eager' : 'lazy'; img.draggable = false;
    fig.appendChild(img);
    galleryStage.appendChild(fig);
...
let gCurrentNatural = null;
function sizeStageToPhoto(w, h){
  if (!w || !h) return;
  gCurrentNatural = [w, h];
  const isLandscape = w > h;
  galleryStage.classList.toggle('is-landscape', isLandscape);
  const vw = window.innerWidth, vh = window.innerHeight;
  const maxW = isLandscape ? Math.min(vw * 0.94, 900) : Math.min(vw * 0.8, 560);
  const maxH = isLandscape ? Math.min(vh * 0.76, 900) : Math.min(vh * 0.66, 700);
  const ratio = w / h;
  let boxW = maxW, boxH = boxW / ratio;
  if (boxH > maxH){ boxH = maxH; boxW = boxH * ratio; }
  galleryStage.style.width = `${boxW}px`;
  galleryStage.style.height = `${boxH}px`;
}
function applyStageRatio(img){ sizeStageToPhoto(img.naturalWidth, img.naturalHeight); }
window.addEventListener('resize', () => {
  if (gCurrentNatural) sizeStageToPhoto(gCurrentNatural[0], gCurrentNatural[1]);
});

function showSlide(i, userTriggered){
  gIndex = (i + PHOTOS.length) % PHOTOS.length;
  const slides = galleryStage.querySelectorAll('.slide');
  slides.forEach((s, k) => {
    if (k === gIndex){
      const img = s.querySelector('img');
      s.classList.remove('is-active');
      img.style.animation = 'none';
      void img.offsetWidth;              // force reflow so the Ken Burns replays
      img.style.animation = '';
      s.classList.add('is-active');
      if (img.complete && img.naturalWidth) applyStageRatio(img);
      else img.addEventListener('load', () => applyStageRatio(img), { once:true });
    } else {
      s.classList.remove('is-active');
    }
  });
  gDots.forEach((d, k) => d.classList.toggle('is-active', k === gIndex));
  galleryCaption.textContent = PHOTOS[gIndex].caption;
  if (userTriggered) restartAutoplay();
}
function gNext(){ showSlide(gIndex + 1); }
function gPrev(){ showSlide(gIndex - 1); }

function startAutoplay(){
  stopAutoplay();
  gTimer = window.setInterval(gNext, 4200);
}
function stopAutoplay(){ if (gTimer){ clearInterval(gTimer); gTimer = 0; } }
function restartAutoplay(){ if (gallery.classList.contains('is-open')) startAutoplay(); }
...
continueBtn.addEventListener('click', openGallery);
galleryClose.addEventListener('click', closeGallery);
galleryNext.addEventListener('click', () => showSlide(gIndex + 1, true));
galleryPrev.addEventListener('click', () => showSlide(gIndex - 1, true));
galleryToFilm.addEventListener('click', () => { closeGallery(); openExtras('reasonsPanel'); });

/* swipe, on the stage only */
let gTouchX = 0, gTouching = false;
galleryStage.addEventListener('pointerdown', (e) => { gTouching = true; gTouchX = e.clientX; });
galleryStage.addEventListener('pointerup', (e) => {
  if (!gTouching) return;
  gTouching = false;
  const dx = e.clientX - gTouchX;
  if (Math.abs(dx) > 40) (dx < 0 ? gNext() : gPrev());
});

function armContinue(){
  continueBtn.hidden = false;
  requestAnimationFrame(() => continueBtn.classList.add('is-shown'));
}


/* ============================================================
   ACT 6 — REASONS → GIFT → LETTER → FINAL
   These chapters deliberately inherit the first site's visual language.
   ============================================================ */
const extras = $('extras');
const reasonsPanel = $('reasonsPanel');
const giftPanel = $('giftPanel');
const letterPanel = $('letterPanel');
...
const birthdayPop = $('birthdayPop');
const giftNext = $('giftNext');
const letterNext = $('letterNext');
const replayWhole = $('replayWhole');
...
const secretConfetti = $('secretConfetti');
const SECRET_PASSWORD = 'bhondu';
const openSecretButton = $('openSecret');
openSecretButton?.addEventListener('click', openSecret);

function openSecret(){
  if (!secretOverlay) return;
  secretOverlay.classList.add('is-open');
  secretOverlay.setAttribute('aria-hidden','false');
  if (secretPassword) setTimeout(() => secretPassword.focus(), 180);
}
...
function launchSecretConfetti(){
  if (!secretConfetti) return;
  secretConfetti.innerHTML='';
  const symbols=['♥','✿','✦','❀'];
  for(let i=0;i<70;i++){
    const p=document.createElement('span');
    p.className='secret-piece';
    p.textContent=symbols[i%symbols.length];
    p.style.left=(Math.random()*100)+'%';
    p.style.setProperty('--drift',((Math.random()-.5)*220)+'px');
    p.style.animationDelay=(Math.random()*.7)+'s';
    p.style.fontSize=(10+Math.random()*14)+'px';
    p.style.color=['#ff8fac','#ffd6a0','#f7d7c4','#fff0f4'][i%4];
    secretConfetti.appendChild(p);
  }
}
if (secretForm){
  secretForm.addEventListener('submit', e => {
    ...
    if (value === SECRET_PASSWORD){
      ...
      if (secretReveal){
        secretReveal.classList.add('is-visible');
        secretReveal.setAttribute('aria-hidden','false');
      }
      const lock=$('secretLock'); if(lock) lock.textContent='💐';
      launchSecretConfetti();
    } else {
      secretError.textContent='Not quite 😌 Try again.';
      if (secretPassword){ secretPassword.value=''; secretPassword.focus(); }
    }
  });
}
secretClose?.addEventListener('click', closeSecret);
secretOverlay?.addEventListener('click', e => { if(e.target===secretOverlay || e.target.classList.contains('secret-backdrop')) closeSecret(); });
document.addEventListener('keydown', e => { if(e.key==='Escape') closeSecret(); });

const finalConfetti = $('finalConfetti');
...
const REASONS = [
  'You make every hangout ten times more fun 🎉',
  'You always know exactly what to say 💬',
  'Your laugh is genuinely contagious 😄',
  'You show up for the people you care about 🤝',
  'You make ordinary moments feel special ✨',
  'You make hard days feel lighter 🌤️',
  'You are unapologetically, wonderfully you 🌟',
  ...
  REASONS.forEach((text, i) => {
    ...
    card.innerHTML = `
      <span class="reason-card__inner">
        <span class="reason-card__front"><b>${String(i + 1).padStart(2,'0')}</b><em>tap me</em></span>
        <span class="reason-card__back">${text}</span>
      </span>`;
    card.addEventListener('click', () => {
      if (card.classList.contains('is-flipped')) return;
      card.classList.add('is-flipped');
      reasonsFlipped++;
      if (reasonsFlipped >= REASONS.length){
        reasonsNext.disabled = false;
        reasonsNext.classList.add('is-ready');
      }
    });
...
function openExtras(panelId='reasonsPanel'){
  buildExtras();
  extras.hidden = false;
  extras.setAttribute('aria-hidden','false');
  document.querySelectorAll('.extra-panel').forEach(p => p.classList.remove('is-active'));
  const panel = $(panelId);
  panel.classList.add('is-active');
  requestAnimationFrame(() => extras.classList.add('is-open'));
}

function closeExtras(){
  extras.classList.remove('is-open');
  extras.setAttribute('aria-hidden','true');
  document.querySelectorAll('.extra-panel').forEach(p => p.classList.remove('is-active'));
}

function goExtra(panelId){
  document.querySelectorAll('.extra-panel').forEach(p => p.classList.remove('is-active'));
  requestAnimationFrame(() => $(panelId).classList.add('is-active'));
}


const LETTER_PARAGRAPHS = [
  `Heyy ❤️`,
  `Aaj aapka birthday hai, so I just wanted to say something jo shayad me normally nhi bol pata.`,
  `Thank you. Srsly. ❤️`,
  `Thank you mere sath rehne ke liye, mujhe support krne ke liye, or sabse zyada mujhe smjhne ki koshish krne ke liye.`,
  `Thank you for worrying about me, even jb me bolta hu “I'm fine” but actually fine nhi hota 😭
Thank you mere nakhre uthane ke liye 😌, meri random bakchodi sunne ke liye, or meri baccho jaisi harkatein tolerate krne ke liye 👉👈`,
  `Or thank you mujhe judge na krne ke liye. I know kbhi kbhi me boht weird, stubborn or annoying ho jata hu, but aap still somehow mujhe handle kr lete ho 😭`,
  `Mujhe sabse zyada ye pasand hai ki aapke samne mujhe pretend nhi krna padta. I can just be myself, stupid jokes, overthinking, random talks, sb kuch.`,
  `Maybe me hr baar properly express nhi kr pata, but I genuinely appreciate you and all the little things you do for me.`,
  `Aapke sath spent ki hui hr choti si memory mere liye special hai. And honestly, I just hope ese hi or boht saare moments me hum sath ho. ❤️`,
  `So today, bas ek cheez yaad rkhna:`,
  `You are really, reallyyyyy special to me😁😁.`,
  `Thank you for being there🥰.
Thank you for understanding me👉👈.
Thank you mere nakhre uthane ke liye 😌
And thank you for being you☺️.`,
  `Happy Birthday, meri favourite person. 😘❤️`,
  `I hope this little surprise makes you smile.
You deserve the best. Always. ❤️`
];

let letterRevealToken = 0;
...
function resetLetter(){
  ...
  letterStatus.textContent = 'writing my heart out for you...';
  letterPaper.classList.remove('is-writing','is-complete');
  letterNext.disabled = true;
  letterNext.classList.remove('is-ready');
}

async function revealLetter(){
  const token = ++letterRevealToken;
  resetLetter();
  // resetLetter increments the token; use the latest token.
  const activeToken = letterRevealToken;
  letterPaper.classList.add('is-writing');
  await sleep(650);
  if (activeToken !== letterRevealToken) return;

  for (let p = 0; p < LETTER_PARAGRAPHS.length; p++){
    if (activeToken !== letterRevealToken) return;
    const paragraph = document.createElement('p');
    paragraph.className = p === 0 ? 'letter-opening' : '';
    letterCopy.appendChild(paragraph);

    const text = LETTER_PARAGRAPHS[p];
    ...
    await sleep(p === 0 ? 250 : 180);
  }

  if (activeToken !== letterRevealToken) return;
  letterSign.textContent = '— with lots of love ❤️';
  letterSign.classList.add('is-written');
  letterStatus.textContent = 'written just for you ♡';
  letterPaper.classList.remove('is-writing');
  letterPaper.classList.add('is-complete');
  letterNext.disabled = false;
  letterNext.classList.add('is-ready');
}

reasonsNext.addEventListener('click', () => goExtra('giftPanel'));

function setGiftRotation(deg){
  giftRotation = deg;
  giftBox.style.transform = `rotateX(-4deg) rotateY(${deg}deg)`;
}

giftScene.addEventListener('pointerdown', e => {
  if (giftOpened) return;
  extrasDragging = true;
  extrasDragX = e.clientX;
  try { giftScene.setPointerCapture(e.pointerId); } catch(_){}
});
giftScene.addEventListener('pointermove', e => {
  if (!extrasDragging || giftOpened) return;
  const dx = e.clientX - extrasDragX;
  if (Math.abs(dx) > 1){
    giftRotation += dx * 0.45;
    setGiftRotation(giftRotation);
    extrasDragX = e.clientX;
  }
});
function endGiftDrag(){ extrasDragging = false; }
giftScene.addEventListener('pointerup', endGiftDrag);
giftScene.addEventListener('pointercancel', endGiftDrag);

function openGift(){
  if (giftOpened) return;
  giftOpened = true;
  giftScene.classList.add('is-opened');
  giftBox.classList.add('is-opened');
  birthdayPop.setAttribute('aria-hidden','false');
  birthdayPop.classList.add('is-visible');
  giftNext.disabled = false;
  giftNext.classList.add('is-ready');
  // confetti-like sparkles
  for (let i=0;i<28;i++){
    const s=document.createElement('i');
    s.className='gift-spark';
    s.style.setProperty('--sx', `${(Math.random()-.5)*260}px`);
    s.style.setProperty('--sy', `${-80-Math.random()*180}px`);
    s.style.setProperty('--delay', `${Math.random()*.35}s`);
    giftScene.querySelector('.gift-sparkles').appendChild(s);
  }
}
giftScene.addEventListener('click', e => {
  if (extrasDragging) return;
  // only open when user taps/clicks after any drag
  openGift();
});
...
replayWhole.addEventListener('click', () => {
  closeExtras();
  resetAll();
});
...
/* ============================================================
   ACT 6b — THE CAKE
   Build it, make a wish, blow the candle, cut it. The flow is a small state
   machine so a stray double-tap can never land mid-animation: every action is
   only honoured from the phase that expects it, and any other phase ignores it.
   ============================================================ */
const cakeStage = $('cakeStage');
const cakeEl = $('cake');
const cakePlate = $('cakePlate');
const cakeHalfL = $('cakeHalfL');
const cakeHalfR = $('cakeHalfR');
const cakeFlameEl = $('cakeFlame');
const cakeSmoke = $('cakeSmoke');
const cakeCandle = $('cakeCandle');
const cakeKnife = $('cakeKnife');
const cakeEyebrow = $('cakeEyebrow');
...
const CAKE_FLOW = {
  idle:    { build:    'wish' },      // cake assembles, candle lights
  wish:    { blow:     'blown' },     // candle is lit and waiting
  blown:   { cut:      'cutting' },   // candle is completely gone, knife comes down
  cutting: { settle:   'done' },      // halves parting
  done:    { continue: 'final' },     // Continue carries on to the finale
};
let cakePhase = 'idle';
let cakeTL = null;
let cakeHandedOff = false;

/* pure — the next phase for an action, or null when the action is out of turn.
   Repeat taps, stale clicks and double-fires all land on null. */
function nextCakePhase(phase, action){
  return (CAKE_FLOW[phase] || {})[action] || null;
}
function cakeAdvance(action){
  const next = nextCakePhase(cakePhase, action);
...
function cakeParts(){
  return {
    body:   cakeHalfL.querySelector('.cake-body'),
    icing:  cakeHalfL.querySelector('.cake-icing'),
    drips:  cakeHalfL.querySelectorAll('.cake-drip'),
    /* the candle is a sibling of the cake, not inside a half: if it lived in
       there the cut would clip it and duplicate it into both slices */
    candle: cakeCandle,
  };
}
function showCakeBtn(btn){
  btn.hidden = false;
  requestAnimationFrame(() => btn.classList.add('is-ready'));
}
function hideCakeBtn(btn){
  btn.hidden = true;
  btn.classList.remove('is-ready');
}

/* the pose the cake starts from, and returns to on replay */
function resetCakeVisuals(){
  if (cakeTL){ cakeTL.kill(); cakeTL = null; }
  const p = cakeParts();
  gsap.killTweensOf([cakeStage, cakePlate, cakeEl, cakeFlameEl, cakeSmoke,
                     cakeKnife, cakeHalfL, cakeHalfR, p.body, p.icing, p.candle]);
  gsap.killTweensOf(p.drips);
  cakeEl.classList.remove('is-cut');
  cakeEl.querySelectorAll('.cake-crumb').forEach(c => c.remove());
  cakeSmoke.classList.remove('is-puffing');
  cakeFlameEl.classList.remove('is-out');
  cakeCandle.hidden = false;          // the blow removes it for real; put it back
  cakeHalfR.innerHTML = '';           // filled from the left half only at the cut

  gsap.set(cakeStage,   { autoAlpha: 0 });
  gsap.set(cakePlate,   { autoAlpha: 0, scaleX: .25, transformOrigin: '50% 50%' });
  gsap.set(p.body,      { autoAlpha: 0, yPercent: 14, scaleY: .18, transformOrigin: '50% 100%' });
  gsap.set(p.icing,     { autoAlpha: 0, yPercent: -46 });
  gsap.set(p.drips,     { scaleY: 0, transformOrigin: '50% 0%' });
  /* clear every axis the blow touches, or the candle comes back squashed */
  gsap.set(p.candle,    { autoAlpha: 0, scaleX: 1, scaleY: 0, xPercent: 0, yPercent: 0,
                          transformOrigin: '50% 100%' });
  /* the blow leaves yPercent and a squashed scaleX/scaleY on this element, so
     every one of those is cleared here or the flame comes back sitting high */
  gsap.set(cakeFlameEl, { autoAlpha: 0, scale: 0, xPercent: 0, yPercent: 0 });
  gsap.set(cakeKnife,   { autoAlpha: 0, xPercent: -50, yPercent: -130 });
  gsap.set([cakeHalfL, cakeHalfR], { xPercent: 0, rotation: 0, transformOrigin: '50% 100%' });
  gsap.set(cakeEyebrow, { autoAlpha: 1 });

  cakeEyebrow.textContent = 'make a wish ✨';
  cakeHint.textContent = '';
  hideCakeBtn(blowBtn);
  hideCakeBtn(cutBtn);
  hideCakeBtn(cakeContinueBtn);
}
...
/* plate → body → icing → drips → candle → flame, in the reference's order */
function buildCake(onBuilt){
  const p = cakeParts();
  cakeTL = gsap.timeline({ onComplete: onBuilt });

  cakeTL
    .set(cakeStage, { autoAlpha: 1 })
    .to(cakePlate, { autoAlpha: 1, scaleX: 1, duration: .6, ease: 'power3.out' })
    .to(p.body, { autoAlpha: 1, yPercent: 0, scaleY: 1, duration: .78,
                  ease: 'back.out(1.5)' }, '-=.32')
    .to(p.icing, { autoAlpha: 1, yPercent: 0, duration: .52, ease: 'power2.in' }, '-=.14')
    .to(p.icing, { yPercent: 4, duration: .16, ease: 'power1.inOut' })
    .to(p.icing, { yPercent: 0, duration: .34, ease: 'back.out(2.4)' })
    .to(p.drips, { scaleY: 1, duration: .42, ease: 'power2.out', stagger: .06 }, '-=.46')
    .to(p.candle, { autoAlpha: 1, scaleY: 1, duration: .5, ease: 'power3.out' }, '-=.18')
    .to(cakeFlameEl, { autoAlpha: 1, scale: 1, duration: .5, ease: 'back.out(2.2)' }, '-=.12');
  return cakeTL;
}

function startCake(){
  if (!cakeAdvance('build')) return;      // already assembling or past this act
  resetCakeVisuals();
  cakePhase = 'wish';
  cakeTL = buildCake(() => {
    if (cakePhase !== 'wish') return;
    cakeHint.textContent = 'make a wish, then blow out the candle 💨';
    showCakeBtn(blowBtn);
  });
}

function blowCandle(){
  if (!cakeAdvance('blow')) return;       // a second tap does nothing
  hideCakeBtn(blowBtn);
  cakeHint.textContent = '';
  cakeFlameEl.classList.add('is-out');    // stops the flicker so the death reads clean

  const p = cakeParts();
  cakeTL = gsap.timeline();
  cakeTL
    /* 1. the flame gutters and goes out */
    .to(cakeFlameEl, { scaleY: .68, duration: .13, ease: 'power1.in' })
    .to(cakeFlameEl, { scaleY: 1.05, duration: .1,  ease: 'power1.out' })
    .to(cakeFlameEl, { scaleY: .72, duration: .1,  ease: 'power1.in' })
    .to(cakeFlameEl, { autoAlpha: 0, scaleX: .55, scaleY: .16,
                       yPercent: -14, duration: .34, ease: 'power2.in' })
    .add(() => cakeSmoke.classList.add('is-puffing'), '-=.26')
    /* 2. then the candle itself — the whole body, not just the flame */
    .to(p.candle, { scaleY: .55, duration: .2, ease: 'power2.in',
                    transformOrigin: '50% 100%' }, '-=.12')
    .to(p.candle, { autoAlpha: 0, scaleY: .1, yPercent: 8, duration: .3, ease: 'power2.in' })
    /* 3. gone in the DOM, not merely faded — so nothing can resurrect or cut it */
    .add(() => { cakeCandle.hidden = true; })
    /* 4. only once it is actually gone does the cut become available */
    .add(() => {
      if (cakePhase !== 'blown') return;
      cakeHint.textContent = 'wish sent ✨';
      showCakeBtn(cutBtn);
    }, '+=.25');
}
...
function crumbBurst(){
  for (let i = 0; i < 16; i++){
    ...
    c.className = 'cake-crumb';
    c.style.top = (34 + Math.random() * 34) + '%';
    cakeEl.appendChild(c);
    gsap.fromTo(c, { xPercent: -50, autoAlpha: 1 },
      { x: (Math.random() - .5) * 96, y: 46 + Math.random() * 74,
        rotation: (Math.random() - .5) * 220, autoAlpha: 0,
        duration: .85 + Math.random() * .6, ease: 'power2.in',
        delay: Math.random() * .12, onComplete: () => c.remove() });
  }
}

function cutCake(){
  if (!cakeAdvance('cut')) return;
  hideCakeBtn(cutBtn);
  cakeHint.textContent = '';

  /* Only the two halves inside .cake are ever touched here. The platform and the
     candle are siblings of .cake, so no transform, clip or overflow on the cake
     can reach them — the platform stays put and the candle stays gone.

     The right half is filled only now. During the build there is a single copy of
     the art, so nothing can double up or fall out of phase; by now the cake is
     still, so the two copies stay pixel-identical. */
  if (!cakeHalfR.childElementCount) cakeHalfR.innerHTML = cakeHalfL.innerHTML;

  cakeTL = gsap.timeline();
  cakeTL
    .to(cakeKnife, { autoAlpha: 1, yPercent: 0, duration: .5, ease: 'power2.in' })
    .add(() => { cakeEl.classList.add('is-cut'); crumbBurst(); })
    .to(cakeHalfL, { xPercent: -7.5, rotation: -2.6, duration: 1.05, ease: 'power3.inOut' })
    .to(cakeHalfR, { xPercent:  7.5, rotation:  2.6, duration: 1.05, ease: 'power3.inOut' }, '<')
    .to(cakeKnife, { autoAlpha: 0, yPercent: -60, duration: .7, ease: 'power2.out' }, '-=.8')
    .add(() => {
      if (cakePhase !== 'cutting') return;
      cakePhase = 'done';
      cakeHint.textContent = 'happiest birthday 🎂';
      showCakeBtn(cakeContinueBtn);    // hand over to the finale on the user's cue
    }, '+=.35');
}

function cakeContinue(){
  if (!cakeAdvance('continue')) return;   // once only, and only after the cut
  hideCakeBtn(cakeContinueBtn);
  if (cakeHandedOff) return;
  cakeHandedOff = true;
  goExtra('finalPanel');
  startFinalConfetti();
}
...
cakeContinueBtn.addEventListener('click', cakeContinue);
/* the flame is a second way in, for taps, and is just as phase-guarded */
cakeFlameEl.addEventListener('click', blowCandle);

resetCake();
function startFinalConfetti(){
  const c=finalConfetti, ctx=c.getContext('2d');
  if (!ctx) return;
  cancelAnimationFrame(finalRAF);
  c.width=window.innerWidth; c.height=window.innerHeight;
  finalPieces=Array.from({length:110},()=>({
    x:Math.random()*c.width, y:-20-Math.random()*c.height,
    vx:(Math.random()-.5)*1.4, vy:1.2+Math.random()*2.2,
    size:4+Math.random()*6, rot:Math.random()*6.28,
    vr:(Math.random()-.5)*.18,
    hue:Math.random()
  }));
  function frame(){
    ...
    for(const p of finalPieces){
      ...
      ctx.fillStyle = p.hue<.33 ? '#ffcf6a' : p.hue<.66 ? '#ff6f97' : '#f7d7c4';
      ctx.fillRect(-p.size/2,-p.size/3,p.size,p.size*.65);
      ctx.restore();
    }
    finalRAF=requestAnimationFrame(frame);
  }
  frame();
}
window.addEventListener('resize', () => {
  if (finalPanel.classList.contains('is-active')) startFinalConfetti();
});

/* When the original tree has settled, its existing "our little gallery"
   bridge remains intact. The gallery's CTA now enters the new chapters. */
